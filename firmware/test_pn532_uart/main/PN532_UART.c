...
uint8_t TX_PIN, RX_PIN, RESET_PIN, IRQ_PIN;
uart_port_t PN532_UART_PORT;
...
void writecommand (uint8_t *cmd, uint8_t cmdlen)
{
  ...
  //Create the command
  uint8_t *command = malloc (UART_BUF_SIZE);
  uint8_t *buffer = malloc (32);

  command[0] = 0x55;
  command[1] = 0x55;
  command[2] = 0x55;
  uart_write_bytes(PN532_UART_PORT, (const char *)command, 3);
  uart_wait_tx_done(PN532_UART_PORT, 1000 / portTICK_PERIOD_MS);
  ...
  command[0] = PN532_PREAMBLE;
  command[1] = PN532_STARTCODE1;
  command[2] = PN532_STARTCODE2;
  command[3] = (cmdlen + 1);
  command[4] = ~(cmdlen + 1) + 1;
  command[5] = PN532_HOSTTOPN532;
  ...
  command[(cmdlen - 1) + 7] = ~checksum;
  command[(cmdlen - 1) + 8] = PN532_POSTAMBLE;
  ...
  */
  ...
  ESP_LOGI(TAG, "%s Sending :", __func__);
  esp_log_buffer_hex(TAG,command,cmdlen+8);

  esp_err_t result = ESP_OK;
  uart_flush_input(PN532_UART_PORT);  
  result = uart_write_bytes(PN532_UART_PORT, (const char *)command, cmdlen + 8);
  uart_wait_tx_done(PN532_UART_PORT, 1000 / portTICK_PERIOD_MS);
  ESP_LOGI(TAG, "Done\n");

  int len = uart_read_bytes(PN532_UART_PORT, buffer, 16, 20 / portTICK_RATE_MS);
  // Start read (n+1 to take into account leading 0x01 with I2C)
  ESP_LOGI(TAG, "Reading: ");
  esp_log_buffer_hex(TAG,buffer,16);
...
/**************************************************************************/
/*!
 @brief  Setups the HW and the UART Bus

 @param  tx      						GPIO PIN for the TX signal
 @param  rx      						GPIO PIN for the RX signal
 @param  irq      					GPIO PIN for the IRQ signal
 @param  uart_port_number   UART Port number

 @return true if hw setup OK, false otherwise
 */
/**************************************************************************/
bool init_PN532_UART (uint8_t tx, uint8_t rx, uint8_t irq, uart_port_t uart_port_number)
{
  TX_PIN = tx;
  RX_PIN = rx;
  IRQ_PIN = irq;
  // RESET_PIN = reset;
  // IRQ_PIN = irq;
  PN532_UART_PORT = uart_port_number;
  ...
  */
  ...
  if (gpio_config (&io_conf) != ESP_OK) {
    ESP_LOGE(TAG, "Unable to configire IRQ pin");    
    return false;
  }
  // Reset the PN532
  // resetPN532 ();
  ...
  ESP_ERROR_CHECK(uart_driver_install(PN532_UART_PORT, UART_BUF_SIZE, 0, 0, NULL, 0));
  ESP_ERROR_CHECK(uart_param_config(PN532_UART_PORT, &uart_config));
  ESP_ERROR_CHECK(uart_set_pin(PN532_UART_PORT, TX_PIN, RX_PIN, -1, -1));
...
bool readdata (uint8_t *buff, uint8_t n)
{
  ...
  esp_err_t result = ESP_OK;
  result = uart_write_bytes(PN532_UART_PORT, command, n + 2);
  if (result != ESP_OK)
  {
    ESP_LOGE(TAG, "Errored i2c_master_cmd_begin ");
        free (buffer);
    return false;
  };
  */

  int len = uart_read_bytes(PN532_UART_PORT, buffer, n + 2, 20 / portTICK_RATE_MS);
...
uint32_t getPN532FirmwareVersion (void)
{
  ...
  {
    ESP_LOGI(TAG, "I2C no acknowledge");
    return 0;
  }
  ...
  {
    ESP_LOGI(TAG, "Firmware does not match!");
    return 0;
  }
  ...
  ESP_LOGI(TAG, "Firmware version: %d", response);
